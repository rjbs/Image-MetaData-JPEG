...
information, such as layers, paths, ecc..., including editorial information
modelled on I<IPTC/NAA> recommendations. This module provides a number of
...
decisions for the end user ....). The structure of the IPTC data block(s)
is managed in detail and separately from the rest, although this block is a
sort of "sub-case" of Photoshop information. The interface is intentionally
similar to L<that|/Exif data ("APP1" segments)> for Exif data.
...
further details):
    ...
    'PHOTOSHOP'   Alpha channels, colour information, transfer functions,
                  and many other details concerning the visual rendering of
                  the picture. This fields are most often only modified by
                  an image manipulation program, and not directly by the user.
